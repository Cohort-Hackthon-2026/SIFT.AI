...
**Project Description:** SIFT.AI (Scoped Intelligence Engine for Legal Research) is an AI-powered legal assistant built on top of Ahnlich AI Infrastructure (an AI vector database/store). Its primary purpose is to address AI hallucinations, context switching, and legal access barriers by offering citation-backed, verifiable answers.
...
results = exa.search(
    "compare recent frontier model launches",
    type="auto",
    num_results=10,
...
```json
{
  "query": "compare recent frontier model launches",
  "type": "deep",
  "systemPrompt": "Prefer official vendor announcements and product release notes. Deduplicate coverage and highlight concrete feature claims.",
  "outputSchema": {
    "type": "object",
    "properties": {
      "summary": {
  ...
  "additionalQueries": [
    "official frontier model launch announcements",
    "frontier AI model release notes",
    "recent flagship model launches"
  ],
  "contents": {
    "highlights": true
  }
...
- When you want to steer deep search toward specific angles, add `additionalQueries` with a few explicit query variants. This is supported only on `deep-lite`, `deep`, and `deep-reasoning`.
